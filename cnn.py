...
# 数据集
tf.flags.DEFINE_string("data_file", "new_data/data_topic_5_0.txt", "Data source for the positive data.")
...
# 多少step测试一次
tf.flags.DEFINE_integer("evaluate_every", 100, "Evaluate model on dev set after this many steps (default: 50)")
# 多少step保存一次模型
...
# 加载word2vec，得到一些信息
filename = '/home/niyao/zhaolei/ZhiHu/data/word_embedding.txt'
def loadword2vec(filename):
    em_0 = list(np.random.uniform(-0.25, 0.25, 256))
    vocab = []
    embd = []
    embd.append(em_0)
...
Weights = tf.Variable(tf.constant(0.0, shape=[len(vocab_processor.vocabulary_), embedding_size]),
                trainable=True, name="Weights")
Weights.assign(embedding)
# Weights = tf.Variable(tf.random_uniform([len(vocab_processor.vocabulary_), embedding_size], -1.0, 1.0), name="Weights")
# Weights.assign(embedding)
...
with tf.name_scope("optimizer"):
    ...
    # learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step, 12000, 0.65, staircase=True)
...
with tf.Session(config=session_conf) as sess:
    ...
    i = 0
    total_steps = (int((len(x_train) - 1) / FLAGS.batch_size) + 1) * FLAGS.num_epochs
    print 'The number of total steps:', total_steps
    # 生成数据
    batches = batch_iter(
    ...
    for batch in batches:
        ...
        # 优化模型 (改二)
        # _, step, lr_rate = sess.run([optimizer, global_step, learning_rate],
